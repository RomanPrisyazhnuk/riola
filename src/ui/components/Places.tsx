      ...
      {places.map((place) => (
        ...
        >
          ...
          <div className="flex flex-col px-2">
            ...
            <p className="text-textColor font-medium text-[16px]">{`${place.excursions_count} экскурсий`}</p>
            <Link className="" aria-label="homepage" href={`/${place.slug}`}>
              <button className="font-medium mt-1 w-full border-2 border-primary rounded-md px-4 py-2 text-primary hover:border-cyan-400">
                Перейти
              </button>
            </Link>
          </div>
