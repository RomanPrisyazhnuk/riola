...
import {Select, SelectItem, Dropdown, DropdownTrigger, DropdownMenu, DropdownItem, Button } from "@nextui-org/react";
import Image from "next/image";
import { mockTransferLocations, TransferLocation } from '@/entities/transferLocation';
...
const TransferSearch: FC<TransferSearchProps> = ({}) => {
  ...
  const [locationFromOptions, setlocationFromOptions] = useState<{key: string, label: string}[]>([]);
  const [selectedLocationFrom, setSelectedLocationFrom] = useState<TransferLocation | null>(null);
  const [locationToOptions, setlocationToOptions] = useState<{key: string, label: string}[]>([]);
  const [selectedLocationTo, setSelectedLocationTo] = useState<TransferLocation | null>(null);

  const prepareTransferLocationsOptions = (transferLocations: TransferLocation[]) =>{
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        const response = await fetch('https://api.riolatravel.com/public/transfers/routes');
        if (!response.ok) {
          throw new Error('Ошибка при загрузке данных');
        }
        const data = await response.json();
        console.log(data);
        // prepareTransferLocationsOptions(mockTransferLocations)
  ...
  const onSelectFromChange = (selectedItemSlug: string) => {
      const location = locations.find((location) => location.slug === selectedItemSlug)
      if(location){
